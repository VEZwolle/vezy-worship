...
export default {
  ...
  created () {
    this.presentation.beamerTitleLines = this.presentation.settings.formatBeamer === 'Geen' ? [] : titleLines(this.presentation.settings.title, this.presentation.settings.formatBeamer, this.font)
    if (this.$store.noLivestream || this.presentation.settings.formatLivestream === 'Geen') {
      this.presentation.sections = splitTextCaption(this.presentation.settings.text, this.presentation.settings.formatBeamer, this.font, 10000)
    } else {
      this.presentation.sections = splitTextCaption(this.presentation.settings.text, this.presentation.settings.formatBeamer, this.font)
    }
  }
}

export function splitTextCaption (text, beamerFormat, beamerFont, maxCharsPerSlide = 350) {
  if (!text) {
    const slides = [['']]
    const beamerLines = [['']]
...
export function titleLines (title, format, font) {
  // for measurement text wrap (same as css)
  let maxWidth = 1000 // fictive width
  ...
  return wrapTextLinesFormat(textToLines(title), maxWidth, font, `${fontSize}px`, `${0.7 * fontSize}px`, `${0.7 * fontSize}px`, fontBold, letterSpacing)
}
</script>
